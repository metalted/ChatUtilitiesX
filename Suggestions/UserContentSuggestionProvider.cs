using ChatUtilities.Data;
using System;
using System.Collections.Generic;
using UnityEngine;

namespace ChatUtilities.Suggestions
{
    public class UserContentSuggestionProvider : IChatSuggestionProvider
    {
        private List<UserContentDefinition> items;

        public string Name
        {
            get
            {
                return "User Content";
            }
        }

        public ChatSuggestionStyle Style { get; private set; }

        public UserContentSuggestionProvider(IEnumerable<UserContentDefinition> userContentDefinitions)
        {
            items = new List<UserContentDefinition>();

            if (userContentDefinitions != null)
            {
                items.AddRange(userContentDefinitions);
            }

            Style = new ChatSuggestionStyle(
                new Color(0.2f, 0.2f, 0.2f, 0.95f),
                new Color(1f, 1f, 1f, 0.05f),
                new Color(1f, 0.3f, 0.3f, 0.3f),
                new Color(0f, 0f, 0f, 0.25f),
                new Vector2(0f, 0.05f),
                new Vector2(1f, 0.75f),
                2f);
        }

        public bool TryGetQuery(string input, out string query)
        {
            query = string.Empty;

            if (string.IsNullOrEmpty(input))
            {
                return false;
            }

            if (!input.StartsWith("*"))
            {
                return false;
            }

            query = input.Substring(1);
            return true;
        }

        public List<ChatSuggestionEntry> GetMatches(string query, int maxResults)
        {
            List<ChatSuggestionEntry> result = new List<ChatSuggestionEntry>();
            string safeQuery = query ?? string.Empty;
            int number;

            if (SuggestionTextUtility.TryParsePositiveInteger(safeQuery, out number))
            {
                AddNumberedEntry(result, number);
                return result;
            }

            for (int i = 0; i < items.Count; i++)
            {
                UserContentDefinition item = items[i];

                if (item == null)
                {
                    continue;
                }

                if (!item.Title.StartsWith(safeQuery, StringComparison.OrdinalIgnoreCase))
                {
                    continue;
                }

                result.Add(CreateEntry(i, item));

                if (result.Count >= maxResults)
                {
                    break;
                }
            }

            return result;
        }

        public string Apply(string input, ChatSuggestionEntry selected)
        {
            if (selected == null)
            {
                return input ?? string.Empty;
            }

            return SuggestionTextUtility.RemovePlaceholders(selected.InsertText);
        }

        private void AddNumberedEntry(List<ChatSuggestionEntry> result, int number)
        {
            int index = number - 1;

            if (index < 0 || index >= items.Count)
            {
                return;
            }

            UserContentDefinition item = items[index];

            if (item == null)
            {
                return;
            }

            result.Add(CreateEntry(index, item));
        }

        private ChatSuggestionEntry CreateEntry(int index, UserContentDefinition item)
        {
            int number = index + 1;
            string displayText = "<color=#fc5d5d>" + number + ". " + item.Title + "</color>\n" + item.Content;
            return new ChatSuggestionEntry(number, item.Title, item.Content, displayText);
        }
    }
}
